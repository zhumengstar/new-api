...
export const calculateModelPrice = ({
  ...
  const userGroupPrices = record.user_group_prices || {};
  const hasUserGroupPrice = (group) =>
    Object.prototype.hasOwnProperty.call(userGroupPrices, group) &&
    Number.isFinite(Number(userGroupPrices[group])) &&
    Number(userGroupPrices[group]) >= 0;
  ...
  if (selectedGroup === 'all' || usedGroupRatio === undefined) {
    // 按次模型有用户专属价格时选择实际费用最低的分组；其余模型沿用最低倍率。
    ...
    if (
      ...
      record.enable_groups.forEach((g) => {
        ...
        const cost =
          record.quota_type === 1 && hasUserGroupPrice(g)
            ? Number(userGroupPrices[g])
            : record.quota_type === 1
              ? Number(record.model_price) * (r ?? 1)
              : (r ?? 1);
        if (cost < minCost) {
          minCost = cost;
          usedGroup = g;
          usedGroupRatio = hasUserGroupPrice(g) ? 1 : (r ?? 1);
        }
      });
    }
  ...
  if (record.quota_type === 1) {
    // 按次计费
    const usesUserPrice = hasUserGroupPrice(usedGroup);
    if (usesUserPrice) {
      usedGroupRatio = 1;
    }
    const priceUSD = usesUserPrice
      ? Number(userGroupPrices[usedGroup])
