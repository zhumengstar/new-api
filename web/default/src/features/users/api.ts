...
export type UserConsumptionStats = {
  daily: Array<{ date: string; quota: number }>;
  today_quota: number;
  total_quota: number;
...
/**
 * Logically delete a single user while retaining the database record.
 */
...
export async function getPerCallModelPrices(): Promise<
  ApiResponse<PerCallModelPrice[]>
> {
